...
<h1 style="display: flex ;justify-content: center;font-family:Microsoft YaHei,
    sans-serif;font-size: 70px;font-weight: 400">学生留言板教师端</h1>
<h2 style="color: #FFFFFF;font-family:Microsoft YaHei, sans-serif;font-weight: 300">回复留言</h2>

<ul class="message-list">
    <c:if test="${not empty MessageList_teacher}">
        <c:forEach var="message" items="${MessageList_teacher}">
            <li>
                <table border="1">
                    <tr>
                        <td>留言ID</td>
                ...
                <c:if test="${message.teacher_id == '20'}">
                    ...
                    <div class="button-container">
                        <form action="${pageContext.request.contextPath}/reply_teacher" method="POST">

                            <input type="hidden" name="messageId" value="${message.id}">
                            <input type="hidden" name="UserID" value="${user.tech_ID}">

                            <label for="replyContent">回复内容：</label>
                            <textarea id="replyContent" name="replyContent" rows="2" cols="50"></textarea>
    ...
    <c:if test="${empty MessageList_teacher}">
        <li>暂无留言</li>
    </c:if>
</ul>
